...
int solution(const std::string &S) {
    ...
    while (iss >> token) {
        if (token == "POP") {
            ...
            if (a > std::numeric_limits<int>::max() - b) return -1; // check for overflow
            stack.push(a + b);
        } else if (token == "-") {
            if (stack.size() < 2) return -1;
            int a = stack.top(); stack.pop();
            int b = stack.top(); stack.pop();
            if (a < b) return -1; // Underflow would happen if a < b
            stack.push(a - b);
        } else {
...
int main() {
    std::cout << solution("4 5 6 - 7 +") << std::endl;           
    std::cout << solution("13 DUP 4 POP 5 DUP + DUP + -") << std::endl; 
    std::cout << solution("5 6 + -") << std::endl;               
    std::cout << solution("3 DUP 5 - -") << std::endl;           
    std::cout << solution("1048575 DUP +") << std::endl;         
